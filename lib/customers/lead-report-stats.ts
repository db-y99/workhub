...
export function calculateLeadReportStats(
  ...
  const enquiry = normalizedStatuses.filter((s) => s === LEAD_STATUS.EQUIRY).length;
  ...
  const application = normalizedStatuses.filter((s) => s === LEAD_STATUS.APPLICATION).length;
  const approved = normalizedStatuses.filter((s) => s === LEAD_STATUS.APPROVED).length;
  const rejected = normalizedStatuses.filter((s) => s === LEAD_STATUS.REJECTED).length;
  const disbursed = normalizedStatuses.filter((s) => s === LEAD_STATUS.DISBURSED).length;
  ...
  const total_disbursed_amount = totalDisbursedAmountRaw > 0 ? totalDisbursedAmountRaw : null;
  const avg_loan_size =
