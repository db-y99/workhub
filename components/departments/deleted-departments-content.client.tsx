...
import { Search, X, RefreshCw, Building2, RotateCcw } from "lucide-react";
...
type DepartmentRow = Department & { isSkeleton?: boolean };

const createSkeleton = (i: number): DepartmentRow => ({
  id: `skeleton-${i}`,
  name: "",
  code: "",
  description: null,
...
export function DeletedDepartmentsContent() {
  ...
  const { data, isLoading, isValidating, mutate } = useSWR<DepartmentsResponse>(
    swrKey,
  ...
  const handleRestore = async (id: string) => {
    ...
    if (result.error) {
      ...
      addToast({ title: "Đã khôi phục phòng ban", color: "success" });
      mutate();
    }
  };
    ...
    <div className="container mx-auto max-w-7xl px-6 py-8">
      <Card className="w-full">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 w-full">
            <div>
              ...
              <p className="text-small text-default-500 mt-1">
                {total > 0 ? `${total} phòng ban đã bị xóa` : "Không có phòng ban nào đã xóa"}
              </p>
            </div>
            <Button
        ...
        <CardBody>
          ...
          <Table aria-label="Danh sách phòng ban đã xóa">
            <TableHeader columns={columns}>
              {(col) => <TableColumn key={col.key}>{col.label}</TableColumn>}
            </TableHeader>
            <TableBody
              items={loading ? Array.from({ length: 5 }, (_, i) => createSkeleton(i)) : departments}
              emptyContent={
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  <Building2 className="text-default-300 mb-4" size={48} />
                  <p className="text-default-500">Không có phòng ban nào đã xóa</p>
                </div>
              }
            >
              {(item: DepartmentRow) => (
                <TableRow key={item.id}>
                  {(columnKey) => {
                    ...
                    if (columnKey === "email") {
                      return <TableCell><span className="text-default-600">{item.email ? highlightSearchText(item.email, debouncedSearch) : "—"}</span></TableCell>;
                    }
